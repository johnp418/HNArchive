...
const HNArchive = (options) => {
        ...
        let reqInterval = setInterval(() => {
          if (urls.length === 0) {
            Promise.all(promises).then((pages) => {
              console.log('All pages are retrieved', pages.length);
   ...
   */
  this._save = (data) => {
    const yearDir = this._getYearDirectory();
    return this._createFolder(yearDir).then((storeDir) => {
  ...
  this._getYearDirectory = (date = null) => {
    date = !date
      ? new Date()
      : date;
  ...
  this.getJSON = (date) => {
    ...
    const filePath = this._getYearDirectory();
  ...
  // Return dir items
  this.getStoredItems = (path = this._getYearDirectory()) => {
  ...
  this.start = (callback, intervalDelay = this.scrapeDelay) => {
    ...
    // Retrieve time in millisec to closest next hour
