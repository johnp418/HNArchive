...
const HNArchive = (options) => {
  ...
  this._process = (pages) => {
	...
	if ($(itemElem).hasClass('athing')) {
	  ...
	  link = link.startsWith('item?id') ? HNLink + link : link;
	  comment = comment.includes('comment') ? comment : null;
	  
  	  let news = { rank, title, link, score, comment, id }
  	  data.push(news);
	}
      });
    });
  ...
  this.getJSON = (date) => {
    ...
    return new Promise((resolve, reject) => {
      this._readFile(path.join(filePath, `${date}.json`))
	  .then((data) => {
  ...
  // TODO: Should this interval kick in at 00:00:00 AM
  this.start = (callback, intervalDelay = this.scrapeDelay) => {
	...
	secToNextHour = 3600 - (currentMin * 60 + currentSec);

    let current = parseInt(date.getTime() / 1000);
    console.log(`Next Scrape in ${new Date((current + secToNextHour) * 1000).toString()}, second left = ${secToNextHour}`);

    let hourTimeout = setTimeout(() => {
      this.scrape().then((data) => {
	console.log('Scrape Done, callback');
	callback(data);
      });
